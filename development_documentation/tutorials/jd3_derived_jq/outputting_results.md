...
All that is left is to tell the job distributor which job results to output and how to do it.
This takes place in `jobs_that_should_be_output()`.

First, we are going to create OutputSpecifications in `note_job_completed()` whenever a node 3 job finishes and store it in a map.
We will then delete these elements from the map when their corresponding job results get deleted.
This may seem odd - why not just wait until `jobs_that_should_be_output()` to create the OutputSpecifications?
It is much easier to create OutputSpecifications when you have access to the LarvalJob object.
We would otherwise have to store all of the LarvalJobs until it is time to output,
so we might as well just store the OutputSpecifications themselves.

We will add elements to the map in `note_job_completed()`,
remove them from the map in `job_results_that_should_be_discarded()`,
and return them to the job distributor in `jobs_that_should_be_output()`.

##Code Additions

###Additions to Header File

We need to add this declaration to the `public` section...

```c++
std::list< jd3::output::OutputSpecificationOP >
jobs_that_should_be_output() override;
```
...which requires this include:

```c++
#include <protocols/jd3/output/OutputSpecification.fwd.hh>
```

I also want this new private data member:
```c++
std::map< jd3::JobResultID, jd3::output::OutputSpecificationOP >
        pose_output_specification_for_job_result_id_;
```

###Addition to note_job_completed()

First off, this is going to require a new include in the .cc file:
```c++
...
###Addition to job_results_that_should_be_discarded()
