<!-- --- title: Make Rot Lib -->Documentation for the MakeRotLib application
...
-   PHI\_RANGE | phi value for this bin phi value for this bin | 0 : The phi range functionality is not functional. Both values need to be the same and the interval set to 0
-   PSI\_RANGE | psi value for this bin psi value for this bin | 0 : The psi range functionality is not functional. Both values need to be the same and the interval set to 0
...
-   rot\_lib\_options\_file: the path to the rotlib options file we made above.
