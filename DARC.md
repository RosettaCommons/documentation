...
Save the names of the compounds in a text file for generating parameters. Next, parameter files for the compounds are generated using batch_molfile_to_params.py which is a python app in rosetta. The command to generate parameter files is:
...
## Building Rosetta with GPU Enabled
...
## Example: Commands for Sample DARC Run for Protein MDM2 PDB:[4ERF|http://www.rcsb.org/pdb/explore/explore.do?structureId=4ERF]
...
Then use the opencl executables instead of default executables with the flag '-gpu 1', i.e call DARC.opencl.macosclangrelease 
instead of DARC.macosclangrelease. All other options are same as CPU.
