...
Aside the beginning step of generating constraints, NodeConstraintAssemblyMover overrides the following base class functions

* **Get starting model** - Finds a starting model that is within the necessary bounds of similarity to the first model from the native

* **Check Edge** - Each edge is checked to ensure that the edges target model satisfies the constraint
