...
- to open ScoreFile plot for `*.score` files: right click on file then select `open` from pop-up menu. In ScoreView you can select which score term to use on X/Y axes, hover over points to view individual scores and double click on point to open particular decoy in your PDB viewer.
