...
```keep_start_selector_rotamers_fixed="True"```: takes the ```start_selector``` positions, fixes their identity and rotamer, turns on proton Chi sampling, thus making them packable (gets them into the IG), as well as allows for more h-bonding possibilities by sampling multiple Hpol positions.  (option added together with Benjamin Basanta)
