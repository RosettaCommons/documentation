...
<HBNetStapleInterface name="(&string;)" hb_threshold="(-0.5 &real;)"
        ...
        scorefxn="(&string;)" combos="(1 &non_negative_integer;)"
        ...
        pH_His="(false &bool;)" only_start_at_interface_pairs="(false &bool;)"
...
-   **seed_hbond_threshold**: (if monte_carlo_branch) Maybe you only want to branch from strong hbonds. If this value is -1.2, then only hbonds with a strength of -1.2 or lower will be branched from.
...
-   **only_start_at_interface_pairs**: Only start IG traversal with h-bonds that span across inerface (different chains)
