...
```
<BfactorFitting name="(string)" wt_adp="(real)" wt_dens="(real)" rmax="(real)" scale="(real)" radius_exp="(non_negative_integer)" max_iter="(non_negative_integer)" minimizer="(string)" init="(bool)" exact="(bool)" verbose="(bool)" deriv_check="(bool)" res_low="(real)" res_high="(real)" nresbins="(non_negative_integer)" weights_to_scan="(real_cslist)" />
```
