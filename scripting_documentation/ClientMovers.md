**ClientMovers** are the modular components that make up any protocol using the Broker framework. ClientMovers produce [[EnvClaim]]s, which communicate to the broker which resources are required. There are several CMs that have already been implemented, and this page describes them. For information about how to write your own novel components for the Broker, check [[Developing for the Broker]].
